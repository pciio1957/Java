...
public class A16_0527ans {
	...
	1. select 문의 기본형식 기술하기
		전체 : select * from 테이블명;
		특정 칼럼: select 컬럼명1, 컬럼명2 from 테이블명;
		where(조건문), group by(그룹합), having(그룹의조건), order by(정렬/desc)
		
	2. 사원정보(emp)에서 부서번호, 사원번호, 사원명, 급여를 출력하세요
		selcet deptno, empno, ename, sal
		from emp;
		
	3. 별칭선언의 기본 형식을 기술하기
		사원정보에서 사원번호를 번호, 사원명을 이름, 급여를 연 봉으로
		alias로 선언하여 출력하기
		select empno "번호", ename "이 름", sal "연 봉"
		from emp
	...
	5. database에 있어서 jdbc는 어떤 역할을 하는가
		java database connection으로 java 프로그램이나
		DB client 툴을 통해서 데이터베이스 서버에 연결해주는 모듈로 구성된 패키지이다.
		web 프로그램의 경우 javaexp\src\main\webapp\WEB-INF\lib에
		프로젝트 단위로 위치시키면 클래스를 객체로 메모리에 로딩시켜서 사용할 수 있다.
	...
	1. 예외의 종류 2가지를 기본 예제를 통해서 기술하기
		1) compile 예외 : 예외를 필수로 처리해야 하는 겅우
				데이터의 io가 일어나는 경우, 파일이 전송되는 경우
				Stream 객체 사용시, network 데이터 전송, DB서버 접속시 
				메모리 로딩 등 
		2) runtime 예외 : 실행 했을 때 실행 오류를 막을 수 있는 예외를 말한다.
	...
	2. 예외 위임이란 무엇인가를 기술하고
		...
		예외 위임 : 현재 메소드에서 처리할 예외를 다른 메소드와 함께
			예외를 처리하게끔 메소드명 옆에 throws 예외명으로 예외를 위임하는 것을 말한다.
