...
5. 뷰 생성 기본
	...
	2) 옵션
		or replace : 뷰를 생성할 때 생성된 뷰가 있으면 동일한 이름을 뷰를 재생성처리
		force : 기본 테이블의 존재 여부에 상관없이 뷰 생성
			ex) 뷰를 먼저 만들고 테이블을 생성할 때 사용
		noforce : 기본테이블이 존재할 경우에만 뷰 생성 
			[기본 값 default]

*/

-- 1981년도에 입사한 사원의 부서번호, 사원번호, 사원명, 사원급여가 있는 view 생성
CREATE VIEW v_emp
AS SELECT deptno, empno, ename, sal
FROM emp 
WHERE to_char(hiredate, 'YYYY') = '1981';

-- view는 실제적으로, 물리적으로 테이블이 만들어지는 것이 아니기에 
-- 기존 emp에 영향을 미치거나 받는다. 
SELECT * FROM v_emp;


-- 응용) 부서별 인원, 최대급여, 최소급여를 뷰로 (v_emp_sum) 만들어 조회하기 (6조)
CREATE VIEW v_emp_sum
AS SELECT deptno, count(deptno) 부서별인원, max(sal) 최대급여, min(sal) 최소급여 
FROM emp
GROUP BY deptno;
...
#인라인뷰(inline View)
- from 절에 참조하는 테이블의 크기가 클 경우 필요한 행과 컬럼만으로 
	구성된 집합을 재정의하여 질의문을 효율적으로 구성하는 것을 말한다 
- from 저에서 서버쿼리를 사용하여 생성한 임시뷰를 말한다.
...
1. 단순 뷰
	하나의 테이블로 생성
	그룹 함수의 사용이 불가능 -> 실제 테이블의 데이터가 아니기때문에 
	distinct 사용일 떄 불가능
...
-- 응용) 사원정보 + 급여등급(salgrade) + 부서정보가 통합된 복합뷰 만들기 (2조)

-- 급여등급 테이블 조회 확인
SELECT * FROM salgrade;

-- 복합뷰 생성 
CREATE VIEW view_emp_sal_dept
AS SELECT e.empno, e.ename, e.sal, s.GRADE, d.deptno, d.dname
FROM emp e, dept d, SALGRADE s 
WHERE e.deptno = d.DEPTNO 
AND sal BETWEEN s.LOSAL AND s.HISAL;

-- 위에서 생성한 복합 뷰 조회 확인
SELECT * FROM view_emp_sal_dept
ORDER BY grade;